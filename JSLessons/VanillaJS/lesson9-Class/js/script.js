// function Car (name,model,year,color){
//     this.ad = name;
//     this.madel= model;
//     this.il= year;
//     this.reng = color;
// }

// Car.prototype.teze = false;
// const myCarData  = new Car("BMW","X5",2002,"black");

// console.log(myCarData.ad);
// console.log(myCarData.teze);

// const Car = (name, model, year, color) => {
//   this.ad = name;
//   this.madel = model;
//   this.il = year;
//   this.reng = color;
// };

// Car.prototype.teze = false;
// const myCarData = new Car("BMW", "X5", 2002, "black");

// console.log(myCarData.ad);
// console.log(myCarData.teze);

// class Car {
//   constructor(name, model, year, color) {
//     this.ad = name;
//     this.madel = model;
//     this.il = year;
//     this.reng = color;
//   }
//   calcualteAge(text){
//     return `${text} : ${this.ad}, is ${new Date().getFullYear()-this.il} `
//   }
// }

// Car.prototype.teze = false;
// const myCarData = new Car("BMW", "X5", 2002, "black");


// console.log(myCarData.ad);
// console.log(myCarData.teze);
// console.log(myCarData.calcualteAge('Car Age'));

// Suretini gostermek isteyirem
// Insan  -> class
// Masin -> class


// class Car {
//   constructor(a){
//     this.b = a;
//     return this.b;

//   };
//   speed(km,hour,item){
//     return `${item} speed: ${km/hour} km/h`
//   }
// }



// class Human extends Car{
//   sayHi(name){
//     return `Hello,${name}`
//   }
// }


// const myCar = new Car("test");
// console.log(myCar);
// console.log(myCar.speed(100,4,"Car"));

// const myHuman = new Human("hello");
// console.log(myHuman);
// console.log(myHuman.speed(100,20,"Human"));
// console.log(myHuman.sayHi("Davud"));









